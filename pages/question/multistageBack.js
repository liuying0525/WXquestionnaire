...
Page(Object.assign({
    ...
    this.getData(uid).then(res => {
      ...
      for (var i = 0; i < dist.length; i++) {
        ...
        for (var k = 0; k < mm.item.length; k++) {
          // if (mm.item[k].item.length!=0){

          // }
          mm.item[k].serial_number = parseInt(mm.item[k].serial_number);
          if (mm.item[k].sub_cat == "multistage") {
            var picList = [];
            var count = parseInt(mm.item[k].option[0].default_choose);
            var resArrary = !!mm.item[k].result ? mm.item[k].result.split(",") : new Array([count]);
            debugger
  ...
  multistageItem: function (modelItem) {
