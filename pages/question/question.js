      ...
      for (var v = 0; v < gData[lindex].item[oindex].item[mindex].option.length; v++) {
        ...
        if (txtValue == gData[lindex].item[oindex].item[mindex].option[v].id) {
          ...
          if (this.data.changeItemModel.option.filter(o => o.id == txtValue).length > 0) {
            var osubidList = gData[lindex].item[oindex].item[mindex].option.filter(b => b.related_sub != "0" && b.related_sub != "");
            //移除当选提有关联选项
            for (var jj = 0; jj < osubidList.length; jj++) {
              ...
              if ((this.data.qmlist[lindex].item[oindex].item.filter(o => o.id == osubidList[jj].related_sub).length>0)&&this.data.qmlist[lindex].item[oindex].item.filter(o => o.id == osubidList[jj].related_sub)[0].is_must == "1") {
      ...
      for (var v = 0; v < gData[lindex].item[oindex].option.length; v++) {
        ...
        if (txtValue == gData[lindex].item[oindex].option[v].id) {
          ...
          if (this.data.changeItemModel.hasOwnProperty("id")) {
            var osubidList = gData[lindex].item[oindex].option.filter(b => b.related_sub != "0" && b.related_sub != "");
            //移除当选提有关联选项
            for (var jj = 0; jj < osubidList.length; jj++) {
              if (!!osubidList[jj].related_sub && osubidList[jj].related_sub != "请选择") {
                ...
                for (var qm = 0; qm < osRelateds.length; qm++) {
                  if (osRelateds[qm] != "") {
                    ...
                    if (this.data.qlist[lindex].item.filter(o => o.id == osRelateds[qm])[0].is_must == "1") {
                      if (mustList.filter(o => o.id == osRelateds[qm]).length == 0) {
                        mustList.push({
                          "id": osRelateds[qm],
                          "questionname": this.data.qlist[lindex].item.filter(o => o.id == osRelateds[qm])[0].title
                        });
                      }
                      debugger
                    }
        ...
        for (var m = 0; m < msmodel.item.length; m++) {
          if (msmodel.item[m].is_must == "1") {
            this.data.mustList.push({
              ...
              "questionname": msmodel.item[m].serial_number+'.'+ msmodel.item[m].title
            });
          }
        }
        if (!!msmodel.mod) {
            ...
            for (var opk = 0; opk < optionRelated.length; opk++) {
              ...
              if (ckRelateId.length == 0) {
                sublist.push({
                  ...
                  "serial_number": modelItem.filter(o => o.id == optionRelated[opk]).length > 0 && modelItem.filter(o => o.id == optionRelated[opk])[0].serial_number
                });
              }
            }
            this.setData({
